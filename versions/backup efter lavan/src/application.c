...
//			float curl = map.waterVel[(x+1)+y*map.w].y - map.waterVel[(x-1)+y*map.w].y - map.waterVel[x+(y+1)*map.w].x + map.waterVel[x+(y-1)*map.w].y;
			...
			if(map.lava[x+y*w].depth > 0.f){
				float lavaConverted = minf(map.lava[x+y*w].depth, maxf((1.f - map.lava[x+y*w].depth/10.f), 0.1f) * dTime);
...
static int mainLoop()
{
    ...
    drawText(topLayer, 10, 130, printfLocal("Water: %f total: %f", map.water[cwx+cwy*map.w].depth, 0));
    drawText(topLayer, 10, 150, printfLocal("Mist: %f total: %f", map.mist[cwx+cwy*map.w].depth, 0));
